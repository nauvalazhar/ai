  ...
  - name: PromptSubmit
    ...
    props:
      - name: render
        ...
        description: Element or function the submit slot renders into. Pass a kit component like `<Button />` to use its styling, or omit to render a plain `<button>`. The slot wires the click handler and `data-slot` onto whatever you render.
---
...
Headless submit slot. Wires the root's submit action onto the click and leaves rendering to the consumer. Pass `render={<Button />}` for the kit's button, or any other element to render into, and supply children for the label and icon.

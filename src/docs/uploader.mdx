...
For the thumbnail to survive reload, the durable source needs to live in `previewUrl`. A server response with `{ url }` flows there automatically (see [Swapping preview on success](#swapping-preview-on-success)). For a fully local demo with no backend, a `FileReader` data URL works in place of a remote URL.

...
parts:
  ...
  - name: TranscriptText
    ...
    props:
      - name: interim
        type: boolean
        description: Mark the text as an in-progress transcription token. Adds `data-interim` and renders the text in italic with a softer color so it reads as not-yet-confirmed. Drop the flag when the transcription engine promotes it to a final.
  - name: TranscriptWord
    passThrough:
...
```tsx
<TranscriptText interim={token.kind === "interim"}>{token.text}</TranscriptText>
```

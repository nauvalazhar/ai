...
parts:
  - name: Tool
    ...
    props:
      - name: state
        type: '"pending" | "approval" | "running" | "success" | "error"'
        description: Lifecycle state for the call. Mirrored onto `data-state` so the trigger pulse, approval ring, spinner, and error slot can react. Leave it undefined for a neutral past call.
  - name: ToolTrigger
    passThrough:
  ...
  - name: ToolArgument
    ...
    props:
      - name: value
        type: string
        description: The raw arguments string. May be incomplete while the model is still streaming tokens. Empty input renders nothing so the panel does not jump as the first character lands.
      - name: state
        type: '"streaming" | "complete"'
        description: Drives the pulse on the last value while tokens are still arriving. Leave undefined for a static, already-complete payload.
---

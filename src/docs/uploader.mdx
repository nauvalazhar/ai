...
parts:
  - name: Uploader
    props:
      - name: uploader
        ...
        description: "The function that actually uploads. Receives the item id, the file, an abort signal, and a progress reporter. Resolves with whatever the backend returns. If the resolved value has a string `url` field, the kit copies it into `previewUrl` on the item, so reloaded items have a durable thumbnail source without holding a `File`."
      - name: accept
        type: string
  ...
  - name: UploadItem
    props:
      ...
      - name: previewUrl
        type: string
        description: "Durable image source. Set automatically when the uploader's resolved value has a string `url` field, so reloaded items show a thumbnail without holding a `File`. Mirrors into `preview` on the merged item."
      - name: preview
        type: string
        description: "Resolved image source for rendering. Equals `previewUrl` when set, otherwise a blob URL the kit manages for the live `File` (image types only). The kit revokes the blob URL when the item is removed or the root unmounts."
      - name: result
        type: unknown
...
The bundled [`createXhrUploader`](#createxhruploader) is a shortcut for the most common case (one multipart POST per file, with progress). Anything beyond that is a normal `async` function. The four arguments above are the only contract; what happens in between is yours.
...
```tsx
const upload: UploaderFn = async ({ file, signal }) => {
...
const upload: UploaderFn = async ({ id, file, signal, onProgress }) => {
  const init = await fetch("/api/uploads/sign", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ id, name: file.name, type: file.type }),
    signal,
  });
  const { putUrl, finalUrl } = await init.json();
...
Throw an `Error` with a `code` property to land a typed error on `item.error`. The kit synthesizes `upload_failed` when no code is provided. Return `{ url }` (or anything with a string `url` field) to set the durable `previewUrl` on success. For chunked or resumable transports, see [Resumable uploads](#resumable-uploads).
...
<Uploader
  ...
  onSuccess={(item) =>
    setItems((prev) =>
      prev.map((i) =>
        i.id === item.id
          ? { ...i, previewUrl: deriveUrl(item.result) }
          : i,
      ),
    )
  }

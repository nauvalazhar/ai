...
Root of the citation popover. Provides the carousel state to every part and tracks the current index, the total number of items, and the direction of the last move so the active item can animate from the matching side.

<Props name="Citation" />

### CitationTrigger

The pill that opens the card on hover, focus, or tap. Wraps a popover trigger with hover-to-open enabled by default, a short delay before opening, and a slightly longer delay before closing, so the gesture survives the user moving from the trigger across the gap into the card. A tap also opens it, which keeps the citation reachable on touch devices that have no hover. Accepts any children, so the author can hand-write the visible label, including an extra count like "+2" when there are more sources behind the trigger.

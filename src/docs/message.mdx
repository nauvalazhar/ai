...
For rich model output (headings, lists, fenced code), drop `Markdown` inside `MessageText`. It renders streaming-safe markdown and routes fenced code through `CodeBlock` so the kit stays cohesive.
